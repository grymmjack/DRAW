...
### BUG-37 [LOW-MED] — INSTANCE bootstrap slot-walk: no mailbox re-clear on the claimed slot; can leave `INST.id` unowned — **FIXED (2026-08-31)**
- When a peer beat us to our auto-assigned slot, the walk-to-next-free-slot updated `INST.mailboxDir`
  and claimed slot `i` but never set `INST.id% = i`. `INSTANCE_clear_my_mailbox` is keyed on
  `INST.id%`, so it cleared the WRONG slot and the walk-claimed slot inherited its predecessor's
  undelivered layers. **FIX:** the walk now adopts the claimed slot as identity (`INST.id% = i`) so
  id / mailboxDir / myRegFile all agree and the correct mailbox is cleared; a fully-lost slot race is
  logged (the extreme all-slots-taken edge is left as-is — not safe to re-plumb unattended). `CORE/INSTANCE.BM`.
...
### BUG-39 [LOW] — Paste/Send Layer stores `blendMode%` with no range clamp (`LAYERXFER.BM:120`). — **FIXED (2026-08-31)** — the value is deserialized from another instance's clipboard/mailbox metadata (untrusted); opacity right above it was clamped 0..255 but blendMode was stored raw, so an out-of-range index would drive the blend `SELECT CASE` / popup into garbage. Now clamped to valid image-layer modes `0 (BLEND_NORMAL) .. 18 (BLEND_LUMINOSITY)`, mirroring the opacity guard.
...
### BUG-69 [MED] — Aseprite compressed-cel `expected_size` LONG overflow (adversarial). — **FIXED (2026-08-31, QB64_GJ_LIB submodule)** — cel width/height are `_UNSIGNED INTEGER` (max 65535), so `expected_size = width*height*bpp` (bpp≤4) can reach ~1.7e10 and overflow the signed 32-bit LONG, wrapping negative/bogus and then driving a huge `STRING$()` pad/alloc. Now computed in `_INTEGER64` with a guard rejecting any cel whose size is non-positive or exceeds LONG range, in both `load_compressed_pixel_data%` and `_for_layer%`. Submodule committed + pushed; DRAW parent pointer bumped.
...
### BUG-76 [LOW] — `.TDX` values trusted without bounds vs the `.TDF` (`TDF-FONT.BM:827`). — **FIXED (2026-08-31)** — each `.TDX` face record carries a `fileOfs`/`blockSz` into the `.TDF`; the render path GETs 188 lookup bytes at `fileOfs+TDF_LOOKUP_OFFSET` and `blockSz` glyph bytes at `fileOfs+TDF_BLOCK_OFFSET` and allocates `SPACE$(blockSz)`. Values were stored verbatim, so a corrupt/hand-crafted `.TDX` could read past EOF (garbage) or allocate a huge string. `TDF_LOOKUP_OFFSET(25)+188 == TDF_BLOCK_OFFSET(213)`, so one extent check covers both reads. **FIX:** load-time validation skips any face whose data block falls outside the font file, plus a defense-in-depth extent re-check at render time. (nameLen/glyph/cell counts are `_UNSIGNED _BYTE`, already bounded.)
### BUG-77 [LOW] — PIXEL-COACH `imgW/imgH` INTEGER overflow on >32767px image. — **FIXED (2026-08-31)** — `COACH.imgW/imgH` and every detector's `w%/h%` locals are INTEGER, so an image over 32767 per side overflows them negative and corrupts every scan. The analyzer is INTEGER throughout and a pixel-art coach has no business on an image that large, so `COACH_load_image` now reads the raw dims as LONG and refuses (logs + clears) an over-32767 image instead of analysing garbage (`PIXEL-COACH/PRECOMPUTE.BM`).
