#!/bin/bash
# =============================================================================
# effect-corona.sh — QA test / regression guard
#
# EFFECTS > SHAPE > Corona... a soft ring of light offset outside the shape's
...
#
# SHAPE is EFFECTS category index 7; Corona is child 1 -> open_effect 7 1.
# =============================================================================

info "=== Effect: Corona Test ==="
...
park_mouse
snap_region "$GX" "$GY" "$GW" "$GH" "corona-before"
BEFORE="$SNAP_RESULT"

open_effect 7 1 ; wait_for 0.5 "Corona dialog open"
screenshot "corona-dialog"
key Return ; wait_for 0.7 "Applied (OK), dialog closed"
assert_no_crash

park_mouse
snap_region "$GX" "$GY" "$GW" "$GH" "corona-after"
AFTER="$SNAP_RESULT"
screenshot "corona-canvas-after"

assert_regions_differ "$BEFORE" "$AFTER" \
    "Corona must add a ring of light around the shape"
assert_no_crash
assert_window_exists
info "=== Effect: Corona Test PASSED ==="
