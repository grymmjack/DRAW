...
## [x] TOOL ACTIVATION AND SWITCHING

### [x] Activating the Text Tool
...
## [x] TEXT ENTRY — ADVANCED

### [x] Auto-Wrap Behavior
...
### [x] State Transition Guard Conditions
...
#### [x] Layer delete during active editing resets text state
1. [x] Start editing text on canvas
2. [x] Delete the current layer (Ctrl+Shift+Delete) while editing
3. [x] Verify text editing exits cleanly (no crash)
4. [x] Verify tool returns to idle state

#### [x] Rapid tool switching doesn't leave stale state
1. [x] Activate text tool, click canvas, type "A"
2. [x] Rapidly press: B (Brush), T (Text), B, T, B, T
3. [x] Verify no ghost text layers or stale cursor states remain
4. [x] Verify text typed in step 2 was committed on first switch

#### [x] Re-enter text tool after cancel
1. [x] Activate text tool, click canvas, type "First"
2. [x] Press Escape to commit
3. [x] Switch to Brush, then back to Text (T)
4. [x] Click canvas to start new text
5. [x] Verify clean state: no leftover cursor from "First", fresh editing

#### [x] Empty text layer re-edit attempt
1. [x] Start text entry (creates text layer)
2. [x] Press Escape immediately (empty layer deleted)
3. [x] Verify clicking where the layer was doesn't cause errors

---

## [x] SOUND FEEDBACK

### [x] Text Tool Sound Effects
Verify sound effects play at appropriate moments.

#### [x] Character typing plays sound
1. [x] Enable sound FX (Audio menu → Sound FX checked)
2. [x] Type characters — verify a typing sound plays per character

#### [x] Enter (newline) plays sound
1. [x] Press Enter during text editing
2. [x] Verify newline sound plays (different from character sound)

#### [x] Backspace plays sound
1. [x] Press Backspace during text editing
2. [x] Verify a deletion sound plays

#### [x] Overflow plays error sound
1. [x] Type text until reaching bottom canvas boundary
2. [x] Verify error/alert sound plays on overflow attempt
