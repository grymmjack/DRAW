...
## 🔨 NOW — doing right now
- [ ] ➡️ **CLI `--help` full coverage**: single test, glob/tag subset, `--all`, `--eta`, `--mode`, `--status`, `--report` (extend the header help added in Phase 2).
...
- [ ] CLI `--help` full coverage: single test, glob/tag subset, `--all`, `--eta`, `--mode`, `--status`, `--report` (extend the header help added in Phase 2).
