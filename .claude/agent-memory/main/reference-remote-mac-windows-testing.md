...
**qb64pe versions (as of 2026-09-12): `local`, `mac`, `thinkpad` are on `v4.6.0-308-g4fa4580ca`; `titan`/`daw` still on `v4.6.0-247-ge048bc093`.** (The old "whole fleet pinned to -247" note is stale — mac/thinkpad/local moved to -308.) Keep the box you build+test on in sync with the source you're validating; unify via git checkout of the target commit + rebuild. `dc` note: `internal/version.txt` says `-UNKNOWN` on git builds — read the real version with `git -C <qb64pe> describe --tags`.
