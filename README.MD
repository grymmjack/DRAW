...
| `--config-upgrade` | Reconcile existing config with latest defaults (adds missing keys) || `--reset-defaults` | Restore factory settings (overwrites config with built-in defaults) |
```bash
